...
def compute_summary(df: pd.DataFrame) -> pd.DataFrame:
    summary = (
        df.groupby(["experiment", "Language", "Program", "PowerLimit"])["power_W"]
        .median()
        .reset_index()
    )
    summary.columns = ["experiment", "language", "benchmark", "power_limit", "median_power_W"]
    return summary
...
def plot_benchmark(summary: pd.DataFrame, benchmark: str, out_dir: Path,
    ...
    fig.suptitle(
        f"Potência mediana — PSys / (User Time + System Time)\nBenchmark: {benchmark}",
    ...
    for exp, powers, ax, title in configs:
        ...
        for i, lang in enumerate(LANGUAGES):
            lang_data = sub[sub["language"] == lang].set_index("power_limit")["median_power_W"]
        ...
        ax.set_ylabel("W (mediana)", fontsize=9)
        ax.set_yscale(scale)
