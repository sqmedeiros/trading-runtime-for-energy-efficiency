...
def plot_benchmark(summary: pd.DataFrame, benchmark: str, out_dir: Path,
    ...
    fig.suptitle(
        f"Potência média (descontando 15% extremos) — PSys\nBenchmark: {benchmark}",
    ...
    for exp, powers, ax, title in configs:
        ...
        for i, lang in enumerate(LANGUAGES):
            if lang == "Python":
                continue
            lang_data = sub[sub["language"] == lang].set_index("power_limit")["mean_PSys"]
        ...
        ax.set_ylabel("Psys (média)", fontsize=9)
        ax.set_yscale(scale)
        #ax.yaxis.set_major_formatter(ticker.FormatStrFormatter("%.1f \u00B5 J"))
        ax.yaxis.set_major_formatter(ticker.FormatStrFormatter("%.1f J"))
